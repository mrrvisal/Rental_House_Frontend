                  ...
                  <div v-if="previews.electric" class="mt-3">
                    <div class="d-flex align-items-start gap-3 flex-wrap">
                      ...
                      <div class="ai-scan-panel ai-scan-panel--electric">
                        ...
                        <div v-if="aiState.electric.loading" class="ai-scan-loading">
                          <div class="ai-scan-pulse ai-scan-pulse--electric"></div>
                          <span>AI កំពុងស្កែន...</span>
                        </div>
                        <div v-if="aiState.electric.value !== null && !aiState.electric.loading" class="ai-scan-result ai-scan-result--electric">
                          ...
                          <div class="ai-scan-result__actions">
                            <button type="button" class="ai-action-btn ai-action-btn--apply" @click="form.new_electric = aiState.electric.value">
                              ✓ ប្រើលេខនេះ
                            </button>
                            <button type="button" class="ai-action-btn ai-action-btn--retry"
                              @click="aiState.electric = { loading: false, value: null, error: null }; runAIScan('electric')">
                              ↺ ស្កែនម្ដងទៀត
                            </button>
                          </div>
                        </div>
                        <div v-if="aiState.electric.error && !aiState.electric.loading" class="ai-scan-error">
                          ...
                          <button type="button" class="ai-action-btn ai-action-btn--retry mt-1"
                            @click="aiState.electric.error = null; runAIScan('electric')">
                            ↺ ព្យាយាមម្ដងទៀត
                          </button>
                        </div>
                      </div>
                  ...
                  <div v-if="previews.water" class="mt-3">
                    <div class="d-flex align-items-start gap-3 flex-wrap">
                      ...
                      <div class="ai-scan-panel ai-scan-panel--water">
                        <button
                          v-if="!aiState.water.loading && aiState.water.value === null && !aiState.water.error"
                          type="button"
                        ...
                        <div v-if="aiState.water.loading" class="ai-scan-loading">
                          <div class="ai-scan-pulse ai-scan-pulse--water"></div>
                          <span>AI កំពុងស្កែន...</span>
                        </div>
                        <div v-if="aiState.water.value !== null && !aiState.water.loading" class="ai-scan-result ai-scan-result--water">
                          <div class="ai-scan-result__label">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                              <polyline points="20 6 9 17 4 12" />
                            </svg>
                          ...
                          <div class="ai-scan-result__actions">
                            <button type="button" class="ai-action-btn ai-action-btn--apply" @click="form.new_water = aiState.water.value">
                              ✓ ប្រើលេខនេះ
                            </button>
                            <button type="button" class="ai-action-btn ai-action-btn--retry"
                              @click="aiState.water = { loading: false, value: null, error: null }; runAIScan('water')">
                              ↺ ស្កែនម្ដងទៀត
                            </button>
                          </div>
                        </div>
                        <div v-if="aiState.water.error && !aiState.water.loading" class="ai-scan-error">
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10" />
                            <line x1="12" y1="8" x2="12" y2="12" />
                            <line x1="12" y1="16" x2="12.01" y2="16" />
                          ...
                          {{ aiState.water.error }}
                          <button type="button" class="ai-action-btn ai-action-btn--retry mt-1"
                            @click="aiState.water.error = null; runAIScan('water')">
                            ↺ ព្យាយាមម្ដងទៀត
                          </button>
                        </div>
                      </div>
...
.ai-scan-btn--electric { background: linear-gradient(135deg, #fbbf24, #f59e0b); color: #78350f; }
.ai-scan-btn--water    { background: linear-gradient(135deg, #38bdf8, #0ea5e9); color: #0c4a6e; }
.ai-scan-btn:hover  { transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,0.18); }
.ai-scan-btn:active { transform: translateY(0); }
...
.ai-scan-pulse--electric { background: linear-gradient(90deg, #fde68a, #f59e0b, #fde68a); background-size: 200% 100%; }
.ai-scan-pulse--water    { background: linear-gradient(90deg, #bae6fd, #0ea5e9, #bae6fd); background-size: 200% 100%; }
...
.ai-scan-result--electric { background: #fffbeb; border-color: #fde68a; }
.ai-scan-result--water    { background: #f0f9ff; border-color: #bae6fd; }
...
.ai-scan-result__actions { display: flex; gap: 6px; flex-wrap: wrap; }
...
.ai-action-btn--apply  { background: #22c55e; color: white; }
.ai-action-btn--apply:hover  { background: #16a34a; }
.ai-action-btn--retry  { background: #f1f5f9; color: #64748b; }
.ai-action-btn--retry:hover  { background: #e2e8f0; }
